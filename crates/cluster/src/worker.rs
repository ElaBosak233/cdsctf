        ...
        loop {
            ...
            for pod in pods {
                let id = pod
                    ...
                    .labels
                    .unwrap_or_default()
                    .get("cds/resource_id").map(|s| s.to_owned()).unwrap_or_default();
                ...
                let renew = annotations.get("cds/renew").map(|s| s.to_owned()).unwrap_or_default().parse::<i64>().unwrap();

                let duration = annotations.get("cds/duration").map(|s| s.to_owned()).unwrap_or_default().parse::<i64>().unwrap();

                let now = chrono::Utc::now().timestamp();

                if now < started_at + renew * duration {
