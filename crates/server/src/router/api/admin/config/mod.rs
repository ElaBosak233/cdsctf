...
pub fn openapi_router(state: Arc<AppState>) -> OpenApiRouter<Arc<AppState>> {
    OpenApiRouter::from(Router::new().with_state(state.clone()))
        ...
        .nest("/logo", logo::openapi_router(state.clone()))
        .nest("/mailbox", email::openapi_router(state.clone()))
}
...
pub async fn get_config(State(s): State<Arc<AppState>>) -> Result<Json<AdminConfigResponse>, WebError> {
